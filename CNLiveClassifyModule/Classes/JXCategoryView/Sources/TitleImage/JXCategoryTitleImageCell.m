...
//        self.imageView.image = [UIImage imageNamed:myCellModel.imageName];
        [self.imageView sd_setImageWithURL:[NSURL URLWithString:myCellModel.imageName]];
    }
...
//        }
//    }
//    NSLog(@">>>>>>>>>>>>> Scale: %f ",myCellModel.titleLabelZoomScale);
    if (myCellModel.titleLabelZoomEnabled) {
        if (myCellModel.titleLabelZoomScale>1.0 && myCellModel.imageName!=nil && myCellModel.imageName.length != 0) {
            self.titleLabel.hidden = YES;
            self.imageView.hidden = NO;
            self.imageView.center = self.contentView.center;
            myCellModel.imageType = JXCategoryTitleImageType_OnlyImage;
        }
    }else {
    ...
    if (myCellModel.imageZoomEnabled) {
        ...
        if (myCellModel.imageZoomScale<1.02) {
            self.imageView.hidden = YES;
            self.titleLabel.hidden = NO;
            self.titleLabel.center = self.contentView.center;
            myCellModel.imageType = JXCategoryTitleImageType_OnlyTitle;
        }
        if (myCellModel.imageZoomScale==1.6) {
            if (self.categoryBaseCellDelegate && [self.categoryBaseCellDelegate respondsToSelector:@selector(refreshTitleViewUIWithIndex:)]) {
                [self.categoryBaseCellDelegate refreshTitleViewUIWithIndex:myCellModel.indexNum];
            }
        }
    }else {
