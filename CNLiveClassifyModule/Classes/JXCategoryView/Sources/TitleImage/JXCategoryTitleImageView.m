...
//    NSLog(@"%ld",(long)index);
//    self.backgroundColor = CNLiveColorWithHexString([self.bgColors safetyObjectAtIndex:index]) ? CNLiveColorWithHexString([self.bgColors safetyObjectAtIndex:index]) : kWhiteColor;
    NSString *statusBarColor = [self.statusBarColors safetyObjectAtIndex:index];
    BOOL lightContent = [statusBarColor isEqualToString:@"0"] ? YES : NO;
    if (self.refreshStatusBarDelegate && [self.refreshStatusBarDelegate respondsToSelector:@selector(refreshStatusBarColorWithLightContent:backgroundColor:)]) {
        [self.refreshStatusBarDelegate refreshStatusBarColorWithLightContent:lightContent backgroundColor:[self.bgColors safetyObjectAtIndex:index]?[self.bgColors safetyObjectAtIndex:index]:@"#FFFFFF"];
    }
}
